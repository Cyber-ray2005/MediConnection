...
import degreeSchema from "../schema/degreeSchema"
...
export default class DegreeDAO {
  ...
  static async getDegree(id) {
    return await degreeSchema.findOne({ _id: ObjectId(id) })
  }
  ...
  static async deleteDegree(id) {
    try {
      const response = await degreeSchema.deleteOne({ _id: ObjectId(id) })
      return { success: true }
    } catch (err) {
