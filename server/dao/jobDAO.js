...
export default class JobDAO {
  ...
  static async getJobs({filter={}, page=0, limit=10}) {
    try {
      const cursor = await jobSchema.find(filter).skip(page*limit).limit(limit)
      return cursor
    } catch (err) {
      return []
    }
  ...
  static async getJob(id) {
    return await jobSchema.findOne({ _id: ObjectId(id) })
  }
  ...
  static async deleteJob(id) {
    try {
      const response = await jobSchema.deleteOne({ _id: ObjectId(id) })
      return { success: true }
    } catch (err) {
