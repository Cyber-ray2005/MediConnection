...
export default class AppointmentDAO {
  ...
  static async getAppointments({filter=null, page=0, limit=10} = {}) {
    try {
      const cursor = await appointmentSchema.find(filter).sort({ "startTime": -1 })
  ...
  static async getAppointment(id) {
    try {
      return await appointmentSchema.findOne({ _id: ObjectId(id) })
    } catch (err) {
  ...
  static async deleteAppointment(id) {
    try {
      await appointmentSchema.deleteOne({ _id: ObjectId(id) })
      return { success: true }
    } catch (err) {
