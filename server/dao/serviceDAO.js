...
export default class ServiceDAO {
  ...
  static async getServices({filter={}, page=0, limit=10}) {
    try {
      ...
      const cursor = await serviceSchema.find(filter).skip(page*limit).limit(limit)
  ...
  static async getService(id) {
    return await serviceSchema.findOne({ _id: ObjectId(id) })
  }
  ...
  static async deleteService(id) {
    try {
      const response = await serviceSchema.deleteOne({ _id: ObjectId(id) })
      return { success: true }
    } catch (err) {
