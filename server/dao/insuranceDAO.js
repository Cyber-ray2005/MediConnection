...
export default class InsuranceDAO {
  ...
  static async getInsurances({filter={}, page=0, limit=10}) {
    try {
      const cursor = await insuranceSchema.find(filter).skip(page*limit).limit(limit)
      return cursor
    } catch (err) {
      return []
    }
  ...
  static async getInsurance(id) {
    return await insuranceSchema.findOne({ _id: ObjectId(id) })
  }
  ...
  static async deleteInsurances(filter) {
    try {
      ...
      await insuranceSchema.deleteMany(
        filter
      )
  ...
  static async deleteInsurance(id) {
    try {
      const response = await insuranceSchema.deleteOne({ _id: ObjectId(id) })
      return { success: true }
    } catch (err) {
